...
class DCGan(BaseModel):
	def generator(self,batch_size,context=None,reuse=False):
		if reuse:
		...
		if context == None:
			...
			self.g_input = tf.concat([self.z,context],1)
			in_dim = self.z_dim+self.context_dim

		# layer 1
		self.g_w1 = tf.get_variable('g_w1',[in_dim,2048],initializer=tf.truncated_normal_initializer(stddev=0.1))
		self.g_b1 = tf.get_variable('g_b1',[2048],initializer=tf.truncated_normal_initializer(stddev=0.1))
		...
		# layer 2
		self.g_w2 = tf.get_variable('g_w2',[2048,1024],initializer=tf.truncated_normal_initializer(stddev=0.1))
		self.g_b2 = tf.get_variable('g_b2',[1024],initializer=tf.truncated_normal_initializer(stddev=0.1))
		...
		# ouput layer
		self.g_w3 = tf.get_variable('g_w3',[1024,self.seq_length],initializer=tf.truncated_normal_initializer(stddev=0.1))
		self.g_b3 = tf.get_variable('g_b3',[self.seq_length],initializer=tf.truncated_normal_initializer(stddev=0.1))
		return tf.nn.tanh(tf.matmul(self.g2,self.g_w3) + self.g_b3)

	def discriminator(self,x,context,reuse=False):
		...
		new_dim = 2 * int(64 * (250/(d_pool_size1*d_pool_size2)))
		self.d2 = tf.reshape(self.d2,[-1,new_dim])
		# context
		self.d2 = tf.concat([self.d2,context],1)
		in_dim = new_dim + self.context_dim
		...
		self.d3 = tf.nn.leaky_relu(tf.matmul(self.d2,self.d_w3) + self.d_b3)
